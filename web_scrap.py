...
from tkinter import Entry, Button, Text, Scrollbar, Label, OptionMenu, StringVar, Radiobutton, filedialog, END
import csv
...
def save_to_csv():
    data_to_save = result_text.get("1.0", END)
    file_path = filedialog.asksaveasfilename(defaultextension=".csv", filetypes=[("CSV Files", "*.csv")])
    if file_path:
        with open(file_path, 'w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=',')
            data = data_to_save.strip().split('\n')
            for row in data:
